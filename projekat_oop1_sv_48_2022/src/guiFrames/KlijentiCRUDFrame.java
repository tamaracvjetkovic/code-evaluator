...
import guiCRUDFrames.KlijentFrameAdd;
import guiCRUDFrames.KlijentFrameEdit;
...
public class KlijentiCRUDFrame extends JFrame {
	...
	public KlijentiCRUDFrame(ManageAll manageAll, String nazivSalona, Menadzer m) {
	...
	private void initialize() {
		...
		setTitle(" " + nazivSalona + " | " + m.getIme() + " " + m.getPrezime() + " - KLIJENTI OPCIJE");
		setSize(880, 410);
		setResizable(false);
		...
		tabela = new JTable(new KlijentiCRUDTableModel(manageAll, 0));		
		...
		btnAdd.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e2) {
				KlijentFrameAdd klijentFrameAdd = new KlijentFrameAdd(manageAll, KlijentiCRUDFrame.this, tabela);
				klijentFrameAdd.toString();
				return;
			}
		});
		...
		btnEdit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e2) {		
				if (tabela.getSelectedRow() != -1) {
					...
					if (k2 != null) {
						KlijentFrameEdit klijentFrameEdit = new KlijentFrameEdit(manageAll, KlijentiCRUDFrame.this, k2, tabela);
						klijentFrameEdit.toString();
						return;
					} else {
						JOptionPane.showMessageDialog(null, "NIJE MOGUĆE PRONAĆI KLIJENTA!", "GREŠKA", JOptionPane.ERROR_MESSAGE);
					}
				} 
			}
		});
			...
			public void actionPerformed(ActionEvent e2) {
				if (tabela.getSelectedRow() != -1) {
					...
					if (k2.getObrisan() == true) {
						JOptionPane.showMessageDialog(null, "KLIJENT JE VEĆ OBRISAN!", "GREŠKA", JOptionPane.WARNING_MESSAGE);
						return;
					}
					int choice = JOptionPane.showConfirmDialog(null, "Potvrdite brisanje klijenta '" + k2.getIme() + " " + k2.getPrezime() + "'. ");
		...
		center(KlijentiCRUDFrame.this, 1);
		setVisible(true);
	}
	
	
	public void refreshData() {
		KlijentiCRUDTableModel sm = (KlijentiCRUDTableModel)this.tabela.getModel();
