...
import guiFrames.KlijentiCRUDFrame;
...
public class KlijentFrameEdit extends JDialog {
	...
	private Klijent k;
	private KlijentiCRUDFrame klijentiCRUDFrame;
	private JTable tabela;
	
	
	public KlijentFrameEdit(ManageAll manageAll, KlijentiCRUDFrame klijentiCRUDFrame, Klijent k, JTable tabela) {
		...
		this.okLozinka = true;
		this.k = k;
		this.klijentiCRUDFrame = klijentiCRUDFrame;
		this.tabela = tabela;
		initialize();
	}
	...
	private void initialize() {
		...
		addWindowListener(new WindowAdapter() {
            ...
            public void windowClosed(WindowEvent e) {
            	((KlijentiCRUDFrame) klijentiCRUDFrame).refreshData();
            	klijentiCRUDFrame.validate();
				klijentiCRUDFrame.repaint();
            }
        });
		
		JLabel editMenadzerTekst = new JLabel("<html> <p style = 'font-size: 13px; font-weight: 500;'> EDIT KLIJENT </p> </html>");
		...
		btnOk.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (okIme == false || okIme == false || okPrezime == false || okPol == false || okTelefon == false || okAdresa == false || okKorisnicko == false || okLozinka == false) {
					...
					String lozinka = lozinkaInput.getText();
					KlijentiCRUDTableModel sm = (KlijentiCRUDTableModel) tabela.getModel();
					sm.edit(k, ime, prezime, pol, telefon, adresa, korisnicko, lozinka, false);
					JOptionPane.showMessageDialog(null, "USPEŠNO IZMENJEN KLIJENT!");
					((KlijentiCRUDFrame) klijentiCRUDFrame).refreshData();
					klijentiCRUDFrame.validate();
					klijentiCRUDFrame.repaint();
					dispose();
		...
		center(KlijentFrameEdit.this, 1);
		setVisible(true);	
	}
