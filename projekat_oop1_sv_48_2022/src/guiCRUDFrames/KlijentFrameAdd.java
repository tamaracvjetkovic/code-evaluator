...
public class KlijentFrameAdd extends JDialog {
	...
	public KlijentFrameAdd(ManageAll manageAll, KlijentiCRUDFrame klijentiCRUDFrame, JTable tabela) {
		...
		btnOk.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (okIme == false || okIme == false || okPrezime == false || okPol == false || okTelefon == false || okAdresa == false || okKorisnicko == false || okLozinka == false) {
					...
					Klijent k2 = new Klijent(id, ime, prezime, pol, telefon, adresa, korisnicko, lozinka, false);	
					KlijentiCRUDTableModel sm = (KlijentiCRUDTableModel) tabela.getModel();
					sm.add(k2);
					JOptionPane.showMessageDialog(null, "USPEŠNO DODAN NOVI KLIJENT!");
					((KlijentiCRUDFrame) klijentiCRUDFrame).refreshData();
					klijentiCRUDFrame.validate();
					klijentiCRUDFrame.repaint();
					dispose();
