...
public class RecepcionerFrameAdd extends JDialog {
	...
	public RecepcionerFrameAdd(ManageAll manageAll, RecepcioneriCRUDFrame recepcioneriCRUDFrame, JTable tabela) {
	...
	private void initialize() {
		...
		JLabel addMenadzerTekst = new JLabel("<html> <p style = 'font-size: 13px; font-weight: 500;'> ADD RECEPCIONER </p> </html>");
		...
		btnOk.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (okIme == false || okIme == false || okPrezime == false || okPol == false || okTelefon == false || okAdresa == false || okKorisnicko == false || okLozinka == false) {
					...
					Recepcioner r2 = new Recepcioner(id, ime, prezime, pol, telefon, adresa, korisnicko, lozinka, nivoStrucneSpreme, staz, bonus, plata, false);	
					RecepcioneriCRUDTableModel sm = (RecepcioneriCRUDTableModel) tabela.getModel();
					sm.add(r2);
					JOptionPane.showMessageDialog(null, "USPEŠNO DODAN NOVI RECEPCIONER!");
					((RecepcioneriCRUDFrame) recepcioneriCRUDFrame).refreshData();
					recepcioneriCRUDFrame.validate();
					recepcioneriCRUDFrame.repaint();
					dispose();
		...
		center(RecepcionerFrameAdd.this, 1);
		setVisible(true);	
	}
