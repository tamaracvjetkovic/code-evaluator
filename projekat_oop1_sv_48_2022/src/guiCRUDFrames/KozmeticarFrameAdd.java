...
public class KozmeticarFrameAdd extends JDialog {
	...
	public KozmeticarFrameAdd(ManageAll manageAll, KozmeticariCRUDFrame kozmeticariCRUDFrame, JTable tabela) {
	...
	private void initialize() {
		...
		JLabel addMenadzerTekst = new JLabel("<html> <p style = 'font-size: 13px; font-weight: 500;'> ADD KOZMETIČAR </p> </html>");
		...
		btnOk.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (okIme == false || okIme == false || okPrezime == false || okPol == false || okTelefon == false || okAdresa == false || okKorisnicko == false || okLozinka == false || okTretmani == false) {
					...
					Kozmeticar k2 = new Kozmeticar(id, ime, prezime, pol, telefon, adresa, korisnicko, lozinka, nivoStrucneSpreme, staz, bonus, plata, tretmani, false);	
					KozmeticariCRUDTableModel sm = (KozmeticariCRUDTableModel) tabela.getModel();
					sm.add(k2);
					JOptionPane.showMessageDialog(null, "USPEŠNO DODAN NOVI KOZMETIČAR!");
					((KozmeticariCRUDFrame) kozmeticariCRUDFrame).refreshData();
					dispose();
				}
			}
		...
		center(KozmeticarFrameAdd.this, 1);
		setVisible(true);	
	}
